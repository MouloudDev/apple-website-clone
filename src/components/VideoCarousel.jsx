...
const VideoCarousel = () => {
  ...
  useGSAP(() => {
    ...
    gsap.to('#video', {
      ...
      onComplete: () => setVideo(prevVideo => ({...prevVideo, startPlay: true, isPlaying: true}))
    })
  }, [isEnd, videoId])
  ...
  const handleProcess = (type, idx) => {
    switch (type) {
      case 'video-end':
        setVideo(prevVideo => ({...prevVideo, isEnd: true, videoId: idx + 1}));
        break;
      case 'video-last':
    ...
    <>
      <div className='flex items-center'>
        {hightlightsSlides.map((list, idx) =>
          <div key={list.id} id='slider' className='sm:pr-20 sm:10'>
